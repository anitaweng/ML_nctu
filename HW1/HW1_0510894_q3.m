...
phi_poly=[ones(size(x_train,1),1),x_train];
phi_gauss=exp(-(x_train-mu).^2./(2*sigma.^2));
phi_gauss=[ones(size(x_train,1),1),phi_gauss];
phi_sig=1./(1.+exp(-(x_train-mu)./sigma));
phi_sig=[ones(size(x_train,1),1),phi_sig];
phi_test_gauss=exp(-(x_val-mu_test).^2./(2*sigma_test.^2));
phi_test_gauss=[ones(size(x_val,1),1),phi_test_gauss];
phi_test_sig=1./(1.+exp(-(x_val-mu_test)./sigma_test));
phi_test_sig=[ones(size(x_val,1),1),phi_test_sig];
phi_test_poly=[ones(size(x_val,1),1),x_val];
...
w_poly=pinv(lambda.*eye(size(phi_poly,2))+phi_poly'*phi_poly)*phi_poly'*y_train;
w_gauss=pinv(lambda.*eye(size(phi_gauss,2))+phi_gauss'*phi_gauss)*phi_gauss'*y_train;
w_sig=pinv(lambda.*eye(size(phi_sig,2))+phi_sig'*phi_sig)*phi_sig'*y_train;
